...
app.MapPost("/api/login", async delegate (HttpContext context, DBConfigurator db)
{
    ...
    if (statusRequestUser.statusCode == 200)
    {
        AccountUser account = db.accountsUser.Include(x => x.dataUser).ThenInclude(x => x.skillsPerson).Include(x => x.dataUser).ThenInclude(x => x.hobbiesPerson).Include(x => x.settingsUser).FirstOrDefault(users => users.settingsUser.hashPassword == Helper.ComputeSHA512(logForm.password) && users.settingsUser.login == logForm.login);
...
app.MapPost("/api/profile/", async delegate (HttpContext context, DBConfigurator db)
{
    ...
    try
    {
        ...
        accountJwt.dataUser.hobbiesPerson = accountUserJson.dataUser.hobbiesPerson;
...
app.MapPost("/api/team/acceptRequire/{idTeam}-{idUser}", async delegate (HttpContext context, DBConfigurator db, int idTeam, int idUser)
{
    ...
    if (teamDb.memberTeam.FirstOrDefault(member => member.roleMember == TypeRoleMember.owner).dataMemberUserId == ownerAccount.dataUserId)
    {
        ...
        await db.SaveChangesAsync();
        return JsonConvert.SerializeObject(db.Teams.Include(team => team.jion_means).Include(team => team.memberTeam).ThenInclude(data => data.dataMemberUser).Include(team => team.team_stack).FirstOrDefault(team => team.id == idTeam));
    }
    else
    {
        context.Response.StatusCode = 403;
...
app.MapPost("/api/team/cancelledRequire/{idTeam}-{idUser}", async delegate (HttpContext context, DBConfigurator db, int idTeam, int idUser)
{
    ...
    if (teamDb.memberTeam.FirstOrDefault(member => member.roleMember == TypeRoleMember.owner).dataMemberUserId == ownerAccount.dataUserId)
    {
        ...
        await db.SaveChangesAsync();
        return JsonConvert.SerializeObject(db.Teams.Include(team => team.jion_means).Include(team => team.memberTeam).Include(team => team.team_stack).FirstOrDefault(team => team.id == idTeam));
    }
    else
    {
        context.Response.StatusCode = 403;
...
{
    AccountUser? ownerAccount = Helper.FindUserFromClaim(context.User.Claims, db);
    Team? team = db.Teams.Include(team => team.team_stack).Include(member => member.memberTeam).ThenInclude(member => member.dataMemberUser).FirstOrDefault(team => team.id == idTeam);
...
app.MapGet("/api/profile/includeTeam/{idAccount}", async delegate (HttpContext context, DBConfigurator db, int idAccount)
{
    //AccountUser? account = Helper.FindUserFromClaim(context.User.Claims, db);
    AccountUser? account = db.accountsUser.Include(acc => acc.dataUser).ThenInclude(data => data.skillsPerson).FirstOrDefault(acc => acc.id == idAccount);
    if (account == null)
    {
        context.Response.StatusCode = 403;
    ...
    return JsonConvert.SerializeObject(db.memberTeams.Include(member => member.team).ThenInclude(team => team.team_stack).Include(team => team.team.memberTeam).ThenInclude(member => member.dataMemberUser).Include(team => team.team.jion_means).Where(member => member.dataMemberUserId == account.id));
...
{
    Team? team = db.Teams.Include(teams => teams.team_stack).Include(teams => teams.jion_means).Include(teams => teams.memberTeam).ThenInclude(member => member.dataMemberUser).FirstOrDefault(teams => teams.id == id);
...
app.MapPost("/api/team/invite/{idDataUserInvited}-{idTeam}", async delegate (HttpContext context, DBConfigurator db, int idDataUserInvited, int idTeam)
{
    ...
    DataUser? dataInvtePerson = db.dataUser.Include(data => data.hobbiesPerson).Include(data => data.skillsPerson).FirstOrDefault(data => data.id == idDataUserInvited);
    Team? targetTeam = db.Teams.Include(team => team.memberTeam).ThenInclude(member => member.dataMemberUser).Include(team => team.team_stack).FirstOrDefault(data => data.id == idTeam);
    ...
    if(dataInvtePerson.invitedFromTeam != null)
    if (dataInvtePerson.invitedFromTeam.Contains(idTeam))
    {
        context.Response.StatusCode = 400;
    ...
    if(dataInvtePerson.invitedFromTeam == null)
    {     
        dataInvtePerson.invitedFromTeam = new List<int>();
    }
        
    dataInvtePerson.invitedFromTeam.Add(targetTeam.id);
    await db.SaveChangesAsync();
...
app.MapPost("api/profile/acceptInvite/{idTeam}", async delegate (HttpContext context, DBConfigurator db, int idTeam)
{
    ...
    Team? team = db.Teams.Include(team => team.team_stack).Include(team => team.memberTeam).ThenInclude(data => data.dataMemberUser).FirstOrDefault(team => team.id == idTeam);
