    ...
    public static class Helper
    {
        ...
        static public AccountUser? FindUserFromClaim(IEnumerable<Claim> claim, DBConfigurator db)
        {
            if (claim.Count() > 0)
            {
                ...
                return db.accountsUser.Include(x => x.dataUser).ThenInclude(x => x.skillsPerson).Include(x => x.dataUser).ThenInclude(x => x.hobbiesPerson).Include(x => x.settingsUser).FirstOrDefault(item => item.settingsUser.login == loginUser);
            }
